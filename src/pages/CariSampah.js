...
const CariSampah = () => {
    ...
    //fetching to get all data of sampah
    useEffect(()=>{
        fetch("https://olwastebe.affaalfiandy.my.id/getdatasampah")
        .then(res=>res.json())
    ...
    const queryHandler = (e) => {
        ...
        setTimeout(()=>{
            setLoading(true)
            fetch("https://olwastebe.affaalfiandy.my.id/getquerysampah",{
                method: 'POST',
                headers: {
    ...
    return(
        <>
            <div className="container justify-content-between w-100 h-75 d-sm-none d-lg-inline-flex">
                <h3 className="vw-75">Cari Sampah</h3>
                <div className="vw-25">
                <SearchBar type="text" placeholder="Cari Sampah" value={query} onChange={queryHandler}/>
                </div>
            </div>
            <br/>
            <br/>
